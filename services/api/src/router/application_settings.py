...
@router.put("/language", status_code=status.HTTP_200_OK, response_model=ApplicationSettings)
async def update_application_language(user: User = Depends(get_current_user), db: Session = db_dependency):
    try:
        user_settings = db.query(SettingsModel).filter(SettingsModel.user_id == user.id).first()

        # もし言語が en あれば ja に、ja なら en に変更する
        if user_settings.language == 'en':
            user_settings.language = 'ja'
        else:
